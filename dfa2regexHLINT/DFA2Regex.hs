...
(Regex), using the dfa2Regex function. This program is based on the Transitive
Closure Method, which is an recursive algorithm to find regular expressions
associated to a deterministic finite automaton. -}
...
{-|Checks if a Regex is written exactly as another Regex.
Does not check equivalence.-}
...
tran map1 = tran' (Map.assocs map1)
...
{- | Extracts necessary information from the given automaton in order to call
the auxiliary function. This function gives support to automata with only one
final state-}
...
dfa2Regex1 (MkDFA q0 d qf) fin =
      ...
      in dfa2Regex' stat (beg, final) (length stat - 1) d

{-| Auxiliary function that applies the "Transitive Closure Method" to the
 given DFA -}
...
dfa2Regex dfa = dfa2Aux dfa (Set.elems (accepting dfa))

{-| Invokes the auxiliary function that uses the Transitive Closure Method and
passes it to the simplify function, in order to reduce the output size.-}
