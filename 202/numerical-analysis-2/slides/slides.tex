...
\aframe{The Navier-Stokes equations for an incompressible fluid for the velocity
  ...
  \end{align*} \pause
  %
  With $f$ is a given force per unit volume and $\sigma(w, p)$ denotes the
  stress tensor given by:
  %
  \begin{equation*}
    \sigma(w, p) = 2\mu\epsilon(w) - pI, \quad
    \epsilon(w) = \frac{1}{2}\left(\nabla w + (\nabla w)^{T}\right)
  \end{equation*}
  %
  $\mu$ is the dynamic viscosity and $I$ is the identity tensor.
}
...
\aframe{Let
  ...
  \end{equation*}
  %
  The Navier-Stokes is solved using a multi-step technique proposed
  by\textcite{goda1979}. \vspace{0.5cm} \pause

  First a tentative velocity $w^{*}$ is calculated by the following
  \textbf{discrete} variational problem:
  %
  \begin{align*}
    &\langle\varrho\Delta t^{-1}(w^{*} - w^{n}), v\rangle +
      \langle\varrho w^{n} \cdot \nabla w^{n}, v \rangle +
      \langle \sigma(w^{n + 0.5}, p^{n}), \epsilon(v) \rangle \\
    &+\langle p^{n}n, v \rangle_{\partial \Omega} -
      \langle \mu n\nabla w^{n + 0.5}, v \rangle_{\partial \Omega}
