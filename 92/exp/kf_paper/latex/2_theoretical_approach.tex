...
\begin{equation}\label{eq:kf_so}
  ...
  \end{split}
\end{equation}
where $\xi_t$ and $\zeta_t$ are random noises.
...
\begin{itemize}
  \item Observed data vector: $y^*_t:=\{y_0^T,\,y_1^T,\dots,y_t^T\}$.
  \item Expected value of $z_t$ given the available data: $z_{t|t-1}:=E(z_t|y^*_{t-1})$.
  \item Expected value of $y_t$ given the available data: $y_{t|t-1}:=E(y_t|y^*_{t-1})$.
  \item $\Delta z_t:=z_t-z_{t|t-1}$.
\end{itemize}
The standard KF
Procedure:
\begin{enumerate}
...
\]
and the previously described KF scheme can be applied. This procedure estimates the state of system. The main problem in this work is that
