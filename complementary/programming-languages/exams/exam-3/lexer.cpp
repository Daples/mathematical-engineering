...
Token* Lexer::nextToken()
{
	...
	if (validVariableName(text)) return new Token(VARIABLE, "");

	// Check if text is a valid constant
	if (validConstant(text, INT)) return new Token(CONSTANT, "");
	if (validConstant(text, FLOAT)) return new Token(CONSTANT, "");
