...
def sweep_noise(vehicles, nodes):
    ...
    if feasible:
        return sol_routes, vehicles, nodes
    else:
...
def grasp(nodes, vehicles, k=10):
    ...
    while aux < k:
        sol, vehicle, node = sweep_noise(vehicles, nodes)
        sols.append(sol)
        complete_vehicles.append(vehicle)
        complete_nodes.append(node)
        aux = sum(sol is not None for sol in sols)
    sols_feasible = []
    complete_vehicles_feasible = []
    complete_nodes_feasible = []
    j = 0
    for sol in sols:
        if sol is not None:
            sols_feasible.append(sol)
            complete_vehicles_feasible.append(complete_vehicles[j])
            complete_nodes_feasible.append(complete_nodes[j])
        j += 1

    return sols_feasible, complete_nodes_feasible, complete_vehicles_feasible
