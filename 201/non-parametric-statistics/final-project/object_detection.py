...
class ImagePreProcessor:
    ...
    def generate_region(self, vertex_sup_left, vertex_bot_right):
        # Find coordinates
        ...
        img_region = Image.fromarray(region, "RGB")

        return (vertex_sup_left, vertex_bot_right, img_region)

    # Adds a region to the regions array
    def add_region(self, vertex_sup_left, vertex_bot_right):
        self.regions.append((vertex_sup_left, vertex_bot_right))

    # Shows the image with a sub-image
    def show(self, region):
        ...
        # Draw sub-image in another axis
        ax2.imshow(region[2])

        # Plot and do not stop running the mode
    ...
    def mode_draw_region(self):
        ...
        while True:
            ...
            while True:
                ...
                region = self.generate_region((x0, y0), (x1, y1))
                plt.close('all')
                self.show(region)
        ...
        print("Exporting information region...")
        try:
            self.export_info()
        except:
            print("Failed! Saving it to temporary file...")
